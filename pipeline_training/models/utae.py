...
class UTAE(nn.Module):
    ...
    def forward(self, x, return_att=False):
        ...
        Args:
            x: dict or tensor
               x["img"]: [B, T, C, H, W]
               x.get("dates"): [B, T] (optional, used as batch_positions)
               or direct tensor [B, T, C, H, W]
        
        Returns:
            dict: {"segmentation": [B, num_classes, H, W]}
        """
        # Extract tensor and dates from dict if needed
        if isinstance(x, dict):
            input_tensor = x["img"]
            batch_positions = x.get("dates", None)
        else:
            input_tensor = x
        ...
        # Create pad mask
        pad_mask = (
            (input_tensor == self.pad_value).all(dim=-1).all(dim=-1).all(dim=-1)
        )  # [B, T]
        pad_mask = torch.zeros_like(pad_mask, dtype=torch.bool)  # disable padding

        # Spatial encoder
        out = self.in_conv.smart_forward(input_tensor)
        feature_maps = [out]
        ...
        if self.encoder:
            return {"segmentation": out, "maps": maps}
        
        out = self.out_conv(out)
        
        if return_att:
            return {"segmentation": out, "attention": att}
        if self.return_maps:
            return {"segmentation": out, "maps": maps}
